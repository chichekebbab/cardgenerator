...
export const getLayoutFilename = (data: CardData): string => {
  ...
  if (
    ...
    data.type === CardType.TREASURE_TRAP
  )
    return 'layout_malediction.png';
  return 'layout_monstre.png';
};

export const getLayoutSource = (data: CardData, settings?: GlobalSettings): string => {
  const filename = getLayoutFilename(data);

  if (settings) {
    if (filename === 'layout_class.png' && settings.customLayoutClass)
      return settings.customLayoutClass;
    if (filename === 'layout_race.png' && settings.customLayoutRace)
      return settings.customLayoutRace;
    if (filename === 'layout_malediction.png' && settings.customLayoutMalediction)
      return settings.customLayoutMalediction;
    if (filename === 'layout_equipement.png' && settings.customLayoutEquipement)
      return settings.customLayoutEquipement;
    if (filename === 'layout_item.png' && settings.customLayoutItem)
      return settings.customLayoutItem;
    if (filename === 'layout_lvlup.png' && settings.customLayoutLvlup)
      return settings.customLayoutLvlup;
    if (filename === 'layout_monstre.png' && settings.customLayoutMonstre)
      return settings.customLayoutMonstre;
  }

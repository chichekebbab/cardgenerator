 ...
 */
export const formatTreasures = (
  count: number,
  t?: (key: string, params?: Record<string, string | number>) => string,
): string => {
  if (t) {
    return count === 1
      ? t('monsterBalance.treasureSingular')
      : t('monsterBalance.treasurePlural', { count });
  }
  if (count === 1) {
    return '1 trésor';
  }
 ...
 */
export const validateMonsterBalance = (
  level: number,
  treasures: string,
  levelsGained: number | '',
  t?: (key: string, params?: Record<string, string | number>) => string,
  ...
  if (levelsGained !== '' && levelsGained !== recommended.levelsGained) {
    const msg = t
      ? t('monsterBalance.recommendedLevels', { count: recommended.levelsGained })
      : `Niveaux recommandés : ${recommended.levelsGained}`;
    warnings.push(msg);
  }
  ...
  if (treasureCount !== null && treasureCount !== recommended.treasuresGained) {
    const msg = t
      ? t('monsterBalance.recommendedTreasures', { count: recommended.treasuresGained })
      : `Trésors recommandés : ${formatTreasures(recommended.treasuresGained)}`;
    warnings.push(msg);
  }
 ...
 */
export const getTargetCountForSlot = (
  slotKey: keyof typeof ITEM_TARGETS,
  deckSize: number,
): number => {

...
import { getLayoutSource, getExportFilename } from '../utils/layoutUtils';
...
interface BatchExportRendererProps {
  ...
  onProgress: (
    current: number,
    total: number,
    chunkInfo?: { chunk: number; totalChunks: number },
  ) => void;
}
...
function waitForImages(container: HTMLElement, timeoutMs = 5000): Promise<void> {
  return new Promise<void>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        console.warn(
          `[Export] Image loading timeout (${timeoutMs}ms) - ${pending} image(s) still pending`,
        );
        done();
      }
    }, timeoutMs);
...
const BatchExportRenderer: React.FC<BatchExportRendererProps> = ({
  cards,
  globalSettings,
  onComplete,
  onProgress,
  ...
  useEffect(() => {
    ...
    const uniqueLayouts = new Set<string>(cards.map((c) => getLayoutSource(c, globalSettings)));
  ...
  useEffect(() => {
    const processNext = async () => {
      ...
      if (currentIndex >= cards.length) {
        ...
        console.log(
          `[Export] All cards processed. Files in final ZIP: ${filesInZip}, total successes: ${successCountRef.current}`,
        );

        if (filesInZip > 0) {
          try {
            const chunkIdx = Math.floor((currentIndex - 1) / CHUNK_SIZE);
            const zipBlob = await zipRef.current.generateAsync({
              type: 'blob',
              compression: 'STORE',
            });
          ...
          showNotifRef.current(
            "❌ Erreur : aucune carte n'a pu être capturée. Vérifiez la console.",
            'error',
          );
        }
      ...
      try {
        ...
        if (!dataUrl || dataUrl.length < 1000) {
          console.error(
            `[Export] Card ${currentIndex} (${cards[currentIndex].title}): toPng returned empty/minimal data (${dataUrl?.length || 0} chars)`,
          );
          ...
          if (exportRef.current) {
            ...
            if (retryDataUrl && retryDataUrl.length >= 1000) {
              console.log(
                `[Export] Card ${currentIndex}: retry succeeded (${retryDataUrl.length} chars)`,
              );
          ...
          if (currentIndex < 5) {
            console.log(
              `[Export] Card ${currentIndex + 1} (${cards[currentIndex].title}) captured in ${Date.now() - t0}ms, dataUrl size: ${dataUrl.length} chars`,
            );
          }
          ...
          if (blob && blob.size > 100) {
            ...
            console.error(
              `[Export] Card ${currentIndex}: blob is empty or too small (${blob?.size || 0} bytes)`,
            );
          }
        }
      } catch (error) {
        console.error(
          `[Export] Error card ${currentIndex} (${cards[currentIndex]?.title}):`,
          error,
        );
      } finally {
        if (!cancelledRef.current) {
          const chunkIdx = Math.floor(currentIndex / CHUNK_SIZE);
          onProgressRef.current(currentIndex + 1, cards.length, {
            chunk: chunkIdx + 1,
            totalChunks,
          });
          ...
          if (isChunkBoundary && currentIndex + 1 < cards.length) {
            try {
              ...
              console.log(
                `[Export] Chunk ${chunkIdx + 1} ZIP generated: ${(zipBlob.size / 1024).toFixed(1)} KB, ${Object.keys(zipRef.current.files).length} files`,
              );
  ...
  }, [currentIndex, cards, totalChunks]);

  if (currentIndex >= cards.length) return null;

  const currentCard = cards[currentIndex];
  const layoutSrc = getLayoutSource(currentCard, globalSettings);

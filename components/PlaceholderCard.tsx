...
const PlaceholderCard: React.FC<PlaceholderCardProps> = ({ level, type, onClick }) => {
  ...
  const getCardTypeLabel = (cardType: CardType) => {
    const key = Object.keys(CardType).find(
      (k) => CardType[k as keyof typeof CardType] === cardType,
    );
    if (!key) return cardType;
  ...
  return (
    <div
      ...
      title={t('placeholderCard.createTitle', { type: getCardTypeLabel(type), level })}
    >
      {/* Icône centrale */}
      <div className="text-4xl text-gray-400 group-hover:text-gray-500 transition-colors">➕</div>
      ...
      <div className="absolute bottom-2 left-2 right-2">
        <div className="bg-gray-200/80 backdrop-blur-sm rounded px-2 py-1 text-center">
          <p className="text-xs font-semibold text-gray-600">{t('placeholderCard.missing')}</p>
          {recommended.exists && (
            <p className="text-xxs text-gray-500">
              {t('placeholderCard.recommendedStats', {
                treasures: recommended.treasuresGained,
                treasurePlural:
                  recommended.treasuresGained > 1
                    ? t('placeholderCard.goldPlural')
                    : t('placeholderCard.goldSingular'),
                levels: recommended.levelsGained,
                levelPlural:
                  recommended.levelsGained > 1
                    ? t('placeholderCard.levelPlural')
                    : t('placeholderCard.levelSingular'),
              })}
            </p>
          )}
        </div>

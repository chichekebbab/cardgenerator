...
export const removeBackground = async (imageBase64: string, apiKey?: string): Promise<string> => {
  ...
  if (!finalApiKey) {
    throw new Error(
      "Clé API remove.bg non configurée. Veuillez l'ajouter dans les paramètres ou configurer l'environnement.",
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        const errorMessage = errorData.errors.map((e: { title: string }) => e.title).join(', ');
    ...
    if (error instanceof Error && error.message.includes('Failed to fetch')) {
      throw new Error(
        "Impossible de contacter l'API remove.bg. Vérifiez votre connexion internet.",
      );
    }
 ...
 * @throws Error if the API request fails
 */
export const removeBackgroundFromUrl = async (
  imageUrl: string,
  apiKey?: string,
): Promise<string> => {
  const finalApiKey = apiKey || import.meta.env.VITE_REMOVE_BG_API_KEY;
